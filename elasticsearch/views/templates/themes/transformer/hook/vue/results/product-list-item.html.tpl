...
<div class="product-container">
  <div class="pro_outer_box">
    <div class="pro_first_box">
      <a class="product_img_link" :href.once="item._source['{Elasticsearch::getAlias('link')|escape:'javascript':'UTF-8'}']"
        ...
        <img class="replace-2x img-responsive front-image"
             ...
             :title.once="item._source['{Elasticsearch::getAlias('name')|escape:'javascript':'UTF-8'}']"
        />
        {*<span v-if="{if $new_sticker != 2}true{else}false{/if} && item._source['{Elasticsearch::getAlias('new')|escape:'javascript':'UTF-8'}']" class="new"><i>{l s='New' mod='elasticsearch'}</i></span>*}
        {*<span v-if="item._source['{Elasticsearch::getAlias('on_sale')|escape:'javascript':'UTF-8'}'] || basePriceTaxIncl > priceTaxIncl" class="on_sale"><i>{l s='Sale'}</i></span>*}
        ...
        {*<span v-if="{if  !isset($restricted_country_mode) && !$PS_CATALOG_MODE && $PS_STOCK_MANAGEMENT}true{else}false{/if} && (item._source['{Elasticsearch::getAlias('show_price')|escape:'javascript':'UTF-8'}'] || item._source['{Elasticsearch::getAlias('available_for_order')|escape:'javascript':'UTF-8'}']">*}
          {*<span v-if="!item._source['{Elasticsearch::getAlias('in_stock')|escape:'javascript':'UTF-8'}'] && !item._source['{Elasticsearch::getAlias('allow_oosp')|escape:'javascript':'UTF-8'}']" class="sold_out">{l s='- Sold out -' mod='elasticsearch'}</span>*}
    ...
    <div class="pro_second_box">
      <h5 itemprop="name"
          class="s_title_block{if isset($length_of_product_name) && $length_of_product_name} nohidden{/if}">
      ...
      {if isset($pro_list_display_brand_name) && $pro_list_display_brand_name}
        <p v-if="item._source['{Elasticsearch::getAlias('manufacturer')|escape:'javascript':'UTF-8'}']" class="pro_list_manufacturer">
