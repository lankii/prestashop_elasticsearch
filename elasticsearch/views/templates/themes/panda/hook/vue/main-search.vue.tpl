  ...
  (function () {
    ...
    new Vue({
      created: function() {
        ...
        ready(function () {
          var searchBox = document.getElementById('submit_searchbox');
          if (searchBox) {
            searchBox.addEventListener('click', function () {
              var searchQueryTopVal = document.getElementById('search_query_top').value.trim();
              if (!searchQueryTopVal || searchQueryTopVal === document.getElementById('search_query_top').getAttribute('placeholder')) {
                document.getElementById('search_query_top').focusout();
                return false;
              }
          ...
          if (typeof window.isPlaceholder === 'function' && !window.isPlaceholer()) {
            var searchQueryTop = document.getElementById('search_query_top');
            if (searchQueryTop) {
              searchQueryTop.addEventListener('focusin', function () {
                if (this.value === this.getAttribute('placeholder')) {
                  this.value = '';
                }
              });
              searchQueryTop.addEventListener('focusout', function () {
                if (!this.value) {
      ...
      methods: {
        ...
        submitHandler: function (event) {
          ...
          if (this.suggestionIndex >= 0
            ...
            window.location.href = this.$store.state.suggestions[this.suggestionIndex]._source['{Elasticsearch::getAlias('link')|escape:'javascript':'UTF-8'}'];
          } else {
            // Go to search page
