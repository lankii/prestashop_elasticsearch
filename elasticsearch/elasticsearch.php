...
class Elasticsearch extends Module
{
    ...
    const SEARCH_MIN = 'ELASTICSEARCH_SEARCH_MIN';
    ...
    public function __construct()
    {
        ...
        $this->description = $this->l('Elasticsearch module for fire team');

        $this->controllers = ['cron', 'proxy', 'search'];
        Configuration::updateGlobalValue(static::SEARCH_MIN, 3);
    ...
    public function install()
    {
        ...
        Configuration::updateGlobalValue(static::INDEX_PREFIX, 'fireteam');
...
//                ],
    ...
    {
        $this->context->smarty->assign([
            'static_token' => sha1(_COOKIE_KEY_.$this->name),
        ]);

        return $this->context->smarty->fetch('module:'.$this->name.'/views/templates/front/ScriptVariables.tpl');
    }
    public function hookDisplayTop()
    {
        return $this->context->smarty->fetch('module:'.$this->name.'/views/templates/hook/top.tpl');
    }
    public static function getStaticToken(){
        return sha1(_COOKIE_KEY_.'elasticsearch');
    }
//    /**
//     * Display top hook
//     *
...
//        // Vue.js
//        $this->context->controller->addJS($this->_path . 'views/js/vue-2.5.11.min.js');
//        $this->context->controller->addJS("https://cdnjs.cloudflare.com/ajax/libs/vue/1.0.18/vue.min.js");
//
//        try {
...
//
//        // Autocomplete CSS
//        if (file_exists(__DIR__ . '/views/templates/themes/classic/front.css')) {
//            $this->context->controller->addCSS(__DIR__ . '/views/templates/themes/classic/front.css');
//        } else {
...
//    }
    ...
    public static function getFrontendHosts()
    {
        try {
            if (Configuration::get(static::PROXY)) {
                return (string) Context::getContext()->link->getModuleLink('elasticsearch', 'proxy', [], Tools::usingSecureMode());
            }
        } catch (PrestaShopException $e) {
            Logger::addLog("Elasticsearch module error: {$e->getMessage()}");
        }
    ...
    public static function tpl($relativePath)
    {
        ...
        $modThemeBaseDir = __DIR__ . '/views/templates/themes/classic/';
